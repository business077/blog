...
function AdminModal({ posts, onClose, onVisitCountChange, onAuthChange, onSaved, onDeleted }) {
  ...
  const resetVisits = async () => {
    if (!window.confirm('Reset the total visit count to zero?')) return;
    setSaving(true);
    setError('');
    try {
      const token = localStorage.getItem('inkwell_admin_token');
      const response = await fetch(apiUrl('/api/visits/reset'), { method: 'POST', headers: { Authorization: `Bearer ${token}` } });
      const data = await response.json();
      if (response.status === 401) { localStorage.removeItem('inkwell_admin_token'); onAuthChange(false); throw new Error('Your admin session expired.'); }
      if (!response.ok) throw new Error(data.message);
      onVisitCountChange(data.total);
    } catch (resetError) { setError(resetError.message); } finally { setSaving(false); }
  };
